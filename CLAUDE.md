...
5. **Page-size seam rule.** These docs are designed for AI-agent consumption, and oversized pages burn agent context. When a page crosses roughly 1,500–2,000 lines, split it along its existing section seams, following the `items.md` → `items-*.md` and `interactions.md` → `interactions-*.md` precedent. **Currently in deliberate arrears** (deferred during the 0.6.3 pass so that splitting did not churn pages while 20+ agents were re-verifying them; sizes refreshed 2026-09-04): `blocks.md` 3,150, `interactions-flow.md` 2,580, `npc-roles.md` 1,919, `ui-api.md` 1,854, `inventory.md` 1,826. Recorded seams: `blocks.md` → the connected-block sections (~380 lines) and `## Java API Reference` onward (~1,100); `inventory.md` → `## Crafting System` onward (~600); `npc-roles.md` → `## Spawn Beacons` through the end of `## SpawnNPC Interaction` (~315 lines, `npc-spawning.md`) and `## Melee attacks without a CAE` + `## Combat Action Evaluator (CAE)` (~217 lines, `npc-combat.md`) — either alone lands the page near 1,600, both near 1,390. `npc-roles.md` was already 1,834 and unrecorded before the 2026-09-04 interaction-coverage pass added a section to it; a page over the threshold that is missing from this list is how an arrear goes quiet, which is what the list exists to prevent.
