...
1. **A registration/type-value oracle.** **Phases (a)–(c) are built as of 2026-09-04** and the vocabulary half is a live hard gate — `check-type-values.py`, run by `verify-docs.sh`: every `"Type"` value in a docs JSON fence must be a name that something registers, resolved against four oracles (all three registration forms mined from the jar; `"Type"` values observed in the shipped assets; **names the page itself registers in its own ```java fences**, because the walkthrough pages are self-registering and a jar-only oracle has a structural false-positive floor no skip list can lower; and one audited entry in `type-value-skiplist.txt`). It found two fabrications on its first run. **Read what it does not cover before trusting a green line**: it checks that a value is registered *somewhere*, not that it is legal in the *slot* it appears in — so it catches invention and is blind to misattribution, and the `"Type": "Wall"` this entry cites as the motivating defect is misattribution — `Wall` is registered on `PatternAsset.CODEC` *and* occurs seven times as a `"Type"` across two shipped assets, so the gate would have passed it for two independent reasons. Scoping it needs a JSON-key→codec binding; the obvious version was measured and is unsound (`Interactions` binds uniquely to `ChoiceInteraction.CODEC`, whose vocabulary is two names, and would fail correct pages), so the scoped half is arrears in `registry-oracle-notes.md`, not half-built behind a warning. The paragraphs below record why the gate is shaped this way and remain the brief for what is left. Registration call sites give the set of legal `"Type"` values per codec, so a fabricated discriminator can be reported with no skip list and no false-positive tail. `"Type"` appears on nearly every JSON page; a fabricated `"Type": "Wall"` shipped for at least a build. **Two premises in the original form of this entry are false, and `maintenance/registry-oracle-notes.md` records the evidence for both — read it before writing the miner.** First, `X.CODEC.register("Name", …)` is not the only registration form: `getCodecRegistry(<codec>).register(…)` chains several names per statement and takes ids that may be constants (or runtime values). Mining only the first form under-counted `Interaction.CODEC` by 28% — 89 of 124 — and hid eleven registries completely. Two further variations cut across *both* forms, so match structure rather than a fixed list: the codec field is not always called `CODEC` (match `\.[A-Z_]*CODEC\b` — `TYPE_CODEC`, `PLUGIN_CODEC` and `PAGE_CODEC` all occur, and `BrushOperation.OPERATION_CODEC` alone carries 56 names), and the name is not always the first argument (`register(Class, "Name", codec)` appears at 18 sites, which is every `GameplayConfig`/`WorldConfig` `PLUGIN_CODEC` registration — a `register\(\s*"` pattern sees none of them). An enumeration here would itself be a closure claim and would rot the same way. Second, a registry is not always *closed*: `MemoriesPlugin.java:137` registers `provider.getId()` in a loop, so `Memory`'s vocabulary cannot be enumerated statically at all. The gate therefore needs three verdicts — closed-and-covered, closed-with-gaps, and **open (not checkable)** — because collapsing the third into the first is a false zero on exactly the registries where a closure claim is least safe.
   ...
   **The defaults half is built too, as of 2026-09-05** — `check-defaults.py`, also a live hard gate: every documented `Default` cell must be the value the field holds when the key is absent. It reads the key→field map out of the chain rather than guessing it (`ClearOutXZ` sets `clearoutXZ`, `DisplayName` sets `displayNameKey`), walks parent chains honouring the parent's *field* name, and refuses — with a stated reason, counted — every key whose setter names no single field. It prints three denominators because they are three different numbers: 60 Default-column key tables on 19 pages, 34 of them in a bound section, 84 comparable rows of 161. Read the narrowing before trusting the green line. **The reason it lands hard rather than behind a warning is measured, not assumed**: across three runs the instrument reported 14, then 8, then 0 disagreements, and all 22 of the first two runs were its own defects — a box read as its primitive, JSON quotes compared as part of the value, an enum-tail rule that took `1.3` apart into `3`. The docs were correct every time. Figures and the full predicate for "Default-column key table" are in `registry-oracle-notes.md` §14, which also settles the 42-vs-53 question §13 left open (it is 60 tables in 40 sections, and the spread came partly from keying sections on their *title*, which is not unique — `interactions-flow.md` has five `#### Core Properties`).
