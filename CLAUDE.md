...
The `docs/` were fact-checked against game **0.5.9** (Update 5 patch; build-22) — `HytaleServer.jar`'s `Implementation-Version` is `0.5.9` (API docs via `javap` on the jar; JSON-asset/DSL docs against the extracted `Assets.zip`). They are only known-accurate as of that build — a game update can silently invalidate them. (The cumulative 0.5.7 → 0.5.9 delta — build-21/0.5.8 was never installed locally, so it was diffed as one step — turned out to be effectively zero for the handbook: asset drift was translation-only, 7 `.lang` files changed content (pt-BR/uk-UA/zh-CN `Server/Languages/*/server.lang` plus the same three locales' Common `avatarCustomization` files), none added or removed, and no doc references any of them (docs cite only `en-US` or the generic path pattern); the entire `com.hypixel.*` API delta was one internal class, `lib.quiche.QuicheNative`, dropping a few native QUIC bindings (keylog/qlog fd, retired-scid) — nothing plugin-facing, nothing documented. The full surface re-passes `verify-docs.sh` against the 0.5.9 jar with all hard gates green.) (Update 5 migrated the math library to JOML — vectors are now `org.joml.*` with Hytale `Vector*Util` companions and `Rotation3f`; see `docs/math.md`.)
