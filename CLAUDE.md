...
1. **A registration/type-value oracle.** Registration call sites give the set of legal `"Type"` values per codec, so a fabricated discriminator can be reported with no skip list and no false-positive tail. `"Type"` appears on nearly every JSON page; a fabricated `"Type": "Wall"` shipped for at least a build. **Two premises in the original form of this entry are false, and `maintenance/registry-oracle-notes.md` records the evidence for both — read it before writing the miner.** First, `X.CODEC.register("Name", …)` is not the only registration form: `getCodecRegistry(<codec>).register(…)` chains several names per statement, takes ids that may be constants (or runtime values), and reaches codec fields not named `CODEC` (`TYPE_CODEC`, `OPERATION_CODEC`, `PAGE_CODEC`). Mining only the first form under-counted `Interaction.CODEC` by 28% — 89 of 124 — and hid eleven registries completely. Second, a registry is not always *closed*: `MemoriesPlugin.java:137` registers `provider.getId()` in a loop, so `Memory`'s vocabulary cannot be enumerated statically at all. The gate therefore needs three verdicts — closed-and-covered, closed-with-gaps, and **open (not checkable)** — because collapsing the third into the first is a false zero on exactly the registries where a closure claim is least safe.
