...
Next tooling levers, in priority order (identified but not built): a **snippet-compilation check** that extracts doc ` ```java ` blocks and compiles them against the jar — the class of bug it catches is real (18 snippets called `sendMessage` on the entity `Player`, which has no such method; only building `examples/events/` exposed it, because the symbol checker cannot type local variables); expanding the coverage advisory's denominator beyond `server.core`/`component`/`math` to the `builtin.*` packages; and generating an `llms.txt` from `docs/01-index.md` + page descriptions for AI-retrieval findability (distribution, not quality, is the open strategic work).
