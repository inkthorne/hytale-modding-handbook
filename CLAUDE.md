...
5. **Page-size seam rule.** These docs are designed for AI-agent consumption, and oversized pages burn agent context. When a page crosses roughly 1,500–2,000 lines, split it along its existing section seams, following the `items.md` → `items-*.md` and `interactions.md` → `interactions-*.md` precedent. **Currently in deliberate arrears** (deferred during the 0.6.3 pass so that splitting did not churn pages while 20+ agents were re-verifying them; sizes refreshed 2026-09-04): `blocks.md` 3,150, `interactions-flow.md` 2,580, `ui-api.md` 1,854, `inventory.md` 1,826, `world.md` 1,760, `prefabs.md` 1,534. Recorded seams: `blocks.md` → the connected-block sections (~380 lines) and `## Java API Reference` onward (~1,100); `inventory.md` → `## Crafting System` onward (~600); `world.md` → two contiguous seams, `## WorldChunk` through `## ChunkTracker` (625 lines, the chunk API) and `## World Events` + `## Chunk Events` (260 lines, 1488–1747), leaving ~875. **`## Chunk Events` goes with `## World Events`, not with the chunk API** — they are adjacent, both document `universe.world.events`/`.events.ecs`, and both are event catalogs, where the chunk sections are class reference. An earlier draft of this entry put it with the chunk material and was wrong. Two traps: the new events page **cannot be called `world-events.md`** — that name is taken by an unrelated subsystem (`builtin.adventure.worldevents`, the 0.6.3 stage/fork timeline; the overlap is the phrase only, verified, not the content), so use something like `world-lifecycle-events.md`; and the split breaks **seven anchor links**, asymmetrically — the chunk seam is the expensive one, not the events seam. Five *other pages* point into it (`universe-saves.md:446`, `collision.md:174`, `prefabs.md:200`, `fluids.md:232`, `blocks.md:120`, all → `world.md#worldchunk`), plus `world.md:134` → `#getchunkflags`, whose source stays in `## World` while its target moves. The events seam costs one line, `world.md:1102` → `#world-events`. `world.md:411` → `#chunk-accessors` **survives** and must not be "fixed": both endpoints move together. An intra-page link breaks only when the seam runs between its two ends. `prefabs.md` → `## Prefab File Format` through `## Integration Points` (~343). **`world.md` and `prefabs.md` were added to this list on 2026-09-04 after being found above the line and unrecorded** — the same way `npc-roles.md` was, and found the same way: by measuring every page rather than re-reading this list. `world.md` is the more urgent of the two because it is the target page for **9 of the 44 undocumented interaction types** (5 of them sections), so the tail will push it further over before it comes back under; decide its split before writing into it, not after. **Watch two more while the tail runs**: `interactions-world.md` (1,378, tied with `world.md` at 9 tail rows) and `items-tools.md` (1,427, 3 rows) are the pages the tail could push over the line — which is the case a periodic measurement catches and a static list never will. **`npc-roles.md` was split on 2026-09-04** and is no longer in arrears: 1,925 → **1,411**, with `## Spawn Beacons` / `## Companion Block Spawners` / `## SpawnNPC Interaction` moved to `npc-spawning.md` (325) and `## Melee attacks without a CAE` / `## Combat Action Evaluator (CAE)` to `npc-combat.md` (227). It had been over the threshold and *unrecorded* since before that pass — a page above the line but missing from this list is how an arrear goes quiet, which is what the list exists to prevent.
6. **Report a denominator, not just findings.** A check that narrates only what it caught cannot be audited: nobody can tell a clean run from a run that examined nothing. Every gate should print what it *scanned* alongside what it *found* — `check-symbols.py`'s `CHECKED_MEMBERS`, the snippet gate's "5 of 1091 java block(s)", the doc-type check's "65 doc(s) scanned, 0 of 1 candidate(s) examined, 1 audited-skip, 3 Package-line citation(s) on 2 page(s) treated as provenance". This is not cosmetic: the snippet gate's green line read as corpus coverage for a year while compiling 3 of 61 stamped pages.
