...
- **`No EntityStatType found for index: %d`** (`%d` is the index) (a `WARNING` log from `EntityStatMap`, not an exception) → an `EntityStatMap` call was given a stat index that no registered stat type maps to (e.g. a hardcoded/stale integer). Fix: obtain indices from `DefaultEntityStatTypes` (`getHealth()`, `getStamina()`, ...) rather than literals.
