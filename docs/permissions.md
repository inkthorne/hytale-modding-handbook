...
- **`Invalid permission node: `** / **`Invalid group name: `** (the offending name follows) (`IllegalArgumentException` from `PermissionsModule.registerPermission`) → the name failed validation. Fix: namespace nodes (`myplugin.feature`) and groups (`myplugin:Role`), and pre-check with `PermissionValidation.isValidPermissionNode` / `isValidGroupName`.
